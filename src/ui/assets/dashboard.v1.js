...
function renderPanelStatus(panelId, payload, error) {
  const panel = document.getElementById(panelId);
  const label = panel.querySelector(".freshness");
  if (error) {
    panel.dataset.state = "stale";
    label.textContent = `${error} · ${panel.dataset.readAt ? formatTime(panel.dataset.readAt) : "no successful read"}`;
    return;
  }
  panel.dataset.readAt = payload.read_at;
  panel.dataset.state = payload.stale ? "stale" : "fresh";
  label.textContent = `${payload.stale ? "stale" : "updated"} · ${formatTime(payload.read_at)}`;
}
...
async function refreshPanel(url, panelId, render) {
  ...
  try {
    ...
    renderPanelStatus(panelId, {}, error instanceof Error ? error.message : "read failed");
  }
}
...
function renderRun(data, payload) {
  ...
  heartbeat.textContent = `Heartbeat ${formatTime(data.heartbeat_at)}`;
...
function refreshPage(name) {
  ...
  return refreshPanel(page.url, `${name}-panel`, rows => {
    ...
    next.disabled = state.rowCount < 100;
  });
}
...
function changePage(name, direction) {
  ...
  if (direction === "next" && state.rowCount === 100) {
    ...
    const cursor = last[state.cursorField];
    state.history.splice(state.index + 1, 1, { url: `/api/${name}?limit=100&${state.cursorKey}=${encodeURIComponent(cursor)}`, cursor: last.id });
    state.index += 1;
  }
  refreshPage(name);
}
...
const jobs = [
  [5000, () => refreshPanel("/api/account", "account-panel", renderAccount)],
  ...
  [5000, () => refreshPanel("/api/run", "run-panel", renderRun)],
