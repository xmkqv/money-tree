...
def _wilder_average(values: pl.Expr, period: int) -> pl.Expr:
    if period <= 0:
...
def _indicator_plan(frame: pl.DataFrame) -> pl.LazyFrame:
    ...
    values = values.with_columns(
        ...
        average_gain=_wilder_average(change.clip(lower_bound=0), N_BAR_RSI),
        average_loss=_wilder_average((-change).clip(lower_bound=0), N_BAR_RSI),
        atr_14=_wilder_average(true_range, N_BAR_ATR),
        average_positive_directional_move=_wilder_average(
            positive_directional_move,
            N_BAR_ADX,
        ),
        average_negative_directional_move=_wilder_average(
            negative_directional_move,
            N_BAR_ADX,
        ),
    )
    ...
    return (
        ...
        .with_columns(adx_14=_wilder_average(pl.col("directional_index"), N_BAR_ADX))
        .select(*output_columns, *INDICATOR_COLUMNS)
    )


def calculate_indicators(frame: pl.DataFrame) -> pl.DataFrame:
    return _indicator_plan(frame).collect()
