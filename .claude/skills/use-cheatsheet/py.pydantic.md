...
[`Field(*, gt, ge, le, min_length, max_length, pattern, default_factory, validation_alias, alias)`][fields] → `FieldInfo`
...
[`model_dump(*, by_alias=False)` / `model_dump_json()` / `model_copy(update=None, deep=False)`][models] → `dict` / `str` / `Self`
